...
public class PathFinding {
     ...
     * @return
     */
    public static List<Point2D> findPath(Grid grid, Point2D startPos, Point2D targetPos, boolean allowDiagonals) {
        // Find path
        List<Node> pathInNodes = findPathNodes(grid, startPos, targetPos, allowDiagonals);

        // Convert to a list of points and return
        List<Point2D> pathInPoints = new ArrayList<Point2D>();

        if (pathInNodes != null)
            for (Node node : pathInNodes)
                pathInPoints.add(new Point2D(node.x, node.y));
     ...
     */
    private static List<Node> findPathNodes(Grid grid, Point2D startPos, Point2D targetPos, boolean allowDiagonals) {
